...
class BookStorageView(QWidget):
    ...
    def init_ui(self):
        ...
        self.search_button.setIcon(QIcon(QPixmap(r'imagesSourse/exit.png')))

        # 查询条件选择按钮
        self.condision_combobox = QComboBox()
        search_condision = ['按书名查询', '按作者查询', '按出版社查询', '按书号查询']
        ...
        self.query_model.setHeaderData(0, Qt.Horizontal, '书号')
        self.query_model.setHeaderData(1, Qt.Horizontal, '书名')
        self.query_model.setHeaderData(2, Qt.Horizontal, '作者')
        self.query_model.setHeaderData(3, Qt.Horizontal, '出版社')
        self.query_model.setHeaderData(4, Qt.Horizontal, '出版时间')
        self.query_model.setHeaderData(5, Qt.Horizontal, '总量')
        self.query_model.setHeaderData(6, Qt.Horizontal, '剩余可解')
    ...
    def get_total_record_count(self):
        self.query_model.setQuery('SELECT BookId ,BookName,Author,Publisher,PublishTime,NumStorage, NumStorage-NumBorrowed FROM book')
    ...
    def record_query(self, index):
        ...
        if condition_choice == '按书名查询':
            condition_choice = 'BookName'
        elif condition_choice == '按作者查询':
            condition_choice = 'Author'
        elif condition_choice == '按出版社查询':
            condition_choice = 'Publisher'
        else:
            condition_choice = 'BookId'

        if self.search_edit.text() == '':
            query_condition = 'SELECT BookId ,BookName,Author,Publisher,PublishTime,NumStorage, NumStorage-NumBorrowed FROM book'
            self.query_model.setQuery(query_condition)
            self.total_record = self.query_model.rowCount()
            self.total_page = int((self.total_record + self.page_record - 1)/self.page_record)
            label = '/' + str(int(self.total_page)) + '页'
            self.pageLabel.setText(label)
            query_condition = "SELECT BookId ,BookName,Author,Publisher,PublishTime,NumStorage, NumStorage-NumBorrowed FROM book ORDER BY %s LIMIT %d, %d"% (condition_choice, index, self.page_record )
            self.query_model.setQuery(query_condition)
        ...
        for i in range(0, len(temp)):
            s = s + temp[i] + '%'
        query_condition = "SELECT BookId ,BookName,Author,Publisher,PublishTime,NumStorage, NumStorage-NumBorrowed FROM book WHERE %s LIKE '%s' ORDER BY %s "%(condition_choice, s, condition_choice)
        self.query_model.setQuery(query_condition)
        self.total_record = self.query_model.rowCount()

        # 如果没有查询到内容
        if self.total_record == 0:
            print(QMessageBox(self, '提醒', '无记录', QMessageBox.Yes, QMessageBox.Yes))
            query_condition = 'SELECT BookId ,BookName,Author,Publisher,PublishTime,NumStorage, NumStorage-NumBorrowed FROM book'
            self.query_model.setQuery(query_condition)
            self.total_record = self.query_model.rowCount()
            self.total_page = int((self.total_record + self.page_record - 1)/self.page_record)
        ...
        label = '/' + str(int(self.total_page)) + '页'
        self.pageLabel.setText(label)
        query_condition = ("SELECT BookId ,BookName,Author,Publisher,PublishTime,NumStorage, NumStorage-NumBorrowed FROM book WHERE %s LIKE '%s' ORDER BY %s LIMIT %d,%d " % (condition_choice, s, condition_choice, index, self.pageRecord))
        self.query_model.setQuery(query_condition)
        self.set_button_status()
        return
